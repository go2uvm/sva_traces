...
// ---------------------------------------------------------
interface accept_test_if (input logic clk);
  ...
  // Create a handle to the actual interface
   ...
   // `uvm_info (log_id, "Fill in your main logic here ", UVM_MEDIUM)
	...
	@(vif.cb);
		...
		vif.cb.done <= 1'b1;
		vif.cb.dma_req <= 1'b1;
		vif.cb.data_transfer <= 1'b1;

		@(vif.cb);
		vif.cb.done <= 1'b1;
		vif.cb.dma_req <= 1'b0;
		vif.cb.data_transfer <= 1'b0;
...
module accept_test_go2uvm;
  ...
  // DUT instance
  accept_test accept_test_0 (.clk(clk),
				.reset_n(accept_test_if_0.reset_n),
				.done(accept_test_if_0.done),
				.dma_req(accept_test_if_0.dma_req),
				.data_transfer(accept_test_if_0.data_transfer)
);
